...
while ~feof(file_handle) %判断是否为文件末尾
    ...
    if contains(line, data_log) %判断是否为数据
        if contains(line, ap_data_log) %判断是否为ap数据
            ...
            ap_num = ap_num + 1;
            data{frame_num}(ap_num).name = name;
            data{frame_num}(ap_num).mac = mac;
            data{frame_num}(ap_num).rssi = str2double(rssi);
            data{frame_num}(ap_num).lat = str2double(lat);
            data{frame_num}(ap_num).lon = str2double(lon);
        end
    end
end
