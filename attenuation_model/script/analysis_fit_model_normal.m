function analysis_fit_model_normal(A, b, rssi, true_dist, varargin)
    ...
    dist_calc = calculate_distance_based_on_rssi(A, b, rssi); % 通过rssi计算出的距离
    ...
    subplot(224)
    model_str_1 = sprintf('$$pa=\\frac{%.4f - rssi}{10*%.4f}$$', A, b);
    % d(rssi) = d_0*10^
    h1 = text(0.1, 0.5, model_str_1);
    model_str_2 = sprintf('$$d(rssi)=d_0*10^{pa}$$');
    h2 = text(0.1, 0.2, model_str_2);
