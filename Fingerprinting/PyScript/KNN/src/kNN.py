...
def datingClassTest():
    ...
    datingDataMat, datingLabels = file2matrix(
        'input/2.KNN/datingTestSet2.txt')  # load data setfrom file
    # 归一化数据
...
def handwritingClassTest():
    ...
    trainingFileList = listdir(
        'input/2.KNN/trainingDigits')  # load the training set
    ...
    for i in range(m):
        ...
        trainingMat[i, :] = img2vector('input/2.KNN/trainingDigits/%s' %
                                       fileNameStr)

    # 2. 导入测试数据
    testFileList = listdir(
        'input/2.KNN/testDigits')  # iterate through the test set
    errorCount = 0.0
